...
func presentadora(canalPresentadora <-chan Aviso, fin chan bool) {
	...
	for nuevoAviso := range canalPresentadora {
		...
		case <-ticker.C:
			if objetivoGala >= nuevoAviso.recaudacion {
				...
				resta := nuevoAviso.recaudacion - objetivoGala
	...
	/*
		// Temporizador para anunciar el total recaudado cada 200 milisegundos
		anunciarTotalTicker := time.Tick(200 * time.Millisecond)

		// Temporizador para verificar donaciones cada 1 segundo
		verificarDonacionesTicker := time.Tick(1 * time.Second)

		var totalRecaudado int       // Variable para almacenar el total recaudado
		var donacionSuperior750 bool // Variable para verificar donaciones superiores a 750€



		for {
			select {
			case nuevoAviso, ok := <-canalPresentadora:
				if !ok {
					// El canal se cerró, salimos del bucle
					<-fin
					return
				}
				totalRecaudado += nuevoAviso.valorDonacion
				donacionSuperior750 = nuevoAviso.valorDonacion < 750
				fmt.Printf("Nuestro operador %d acaba de recibir una generosa donación de %d€!\n", nuevoAviso.numOperador, nuevoAviso.valorDonacion)
			case <-anunciarTotalTicker:
				// Anunciar el total recaudado cada 200 milisegundos
				fmt.Printf("Total recaudado hasta ahora: %d€\n", totalRecaudado)
			case <-verificarDonacionesTicker:
				// Verificar si ha pasado 1 segundo sin donaciones superiores a 750€
				if !donacionSuperior750 {
					// Decir una frase para alentar donaciones
					fmt.Println("Continúen llamando para hacer sus donaciones por esta buena causa")
				}
				// Reiniciar la bandera
				donacionSuperior750 = false
			}
		}*/
}
